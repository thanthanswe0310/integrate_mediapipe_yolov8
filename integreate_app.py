...
def main():
    ...
    # Load YOLOv8 model
    model = YOLO('/home/tricubics/Desktop/integrate_mediapipe_yolov8/yolov8_best.pt')

    if os.path.isfile(image_dir):  # Only if image_dir is a file
        # Load template image
        template_image = cv.imread(image_dir, cv.IMREAD_GRAYSCALE)
        if template_image is None:
            print(f"Error: The image at {image_dir} could not be loaded.")
            return
        template_width, template_height = template_image.shape[::-1]
    else:
        # Handle case where image_dir is a directory
        template_image = None
        template_width = template_height = 0
    ...
    for image_path in image_paths:
        ...
        if template_image is not None:
            ...
            res = cv.matchTemplate(gray_image, template_image, cv.TM_CCOEFF_NORMED)
            min_val, max_val, min_loc, max_loc = cv.minMaxLoc(res)
            threshold = 0.8  # Adjust the threshold according to your needs
            if max_val >= threshold:
                top_left = max_loc
                bottom_right = (top_left[0] + template_width, top_left[1] + template_height)
                cv.rectangle(debug_image, top_left, bottom_right, (0, 255, 0), 2)
                cv.putText(debug_image, 'Template Found', (top_left[0], top_left[1] - 10), cv.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
